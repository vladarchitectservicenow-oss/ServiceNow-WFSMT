...
Build a production-ready ServiceNow scoped application that maps legacy workflows and flow designer flows to the new workflow studio .now.ts dsl, calculating migration velocity, confidence scores, and auto-generating dsl templates.. The application must be installable on any Australia-era ServiceNow instance and must pass automated functional tests with zero external runtime dependencies.
